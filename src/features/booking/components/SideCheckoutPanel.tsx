  ...
  return createPortal(
    <>
      ...
      <div style={{
        ...
        animation:'svcIn 0.2s cubic-bezier(0.25,0.46,0.45,0.94)',
      }}>
        <style>{`@keyframes svcIn{from{transform:translateX(100%)}to{transform:translateX(0)}}`}</style>
        ...
        <div style={{ padding:'16px 20px', borderBottom:`1px solid ${colors.gray.border}`, display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
          ...
          <button onClick={onClose} style={{ width:28, height:28, borderRadius:radius.full, border:`1px solid ${colors.gray.borderMd}`, background:colors.background.surfaceLight, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', transition:transitions.fast }}>
        ...
        <div style={{ flex:1, overflowY:'auto' }}>
          {loading ? (
            <div style={{ padding:32, textAlign:'center', color:colors.gray.dimText, fontSize:typography.scale.base }}>Carregando...</div>
          ) : services.length === 0 ? (
            <div style={{ padding:32, textAlign:'center', color:colors.gray.dimText, fontSize:typography.scale.base }}>Nenhum serviço cadastrado</div>
          ) : services.map(s => (
            ...
            >
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ fontSize:typography.scale.sm, color:colors.gray.dimText, marginTop:2, display:'flex', alignItems:'center', gap:4 }}>
                  ...
                  {s.duration}min
                  {s.price != null && <span style={{ marginLeft:4, fontWeight:typography.weight.medium }}>· R$ {s.price.toFixed(2).replace('.', ',')}</span>}
                </div>
              </div>
              {selected?.id === s.id
...
export default function SideCheckoutPanel({
  ...
  useEffect(() => {
    if (!open) return
    setTab('booking'); setError(null); setSuccess(false); setShowSvcSheet(false)
    ...
    <>
      <style>{`
        ...
        .cp-input {
          ...
          color:${colors.gray[900]}; font-size:${typography.scale.base}px;
        ...
        .cp-prof {
          padding:6px 14px; border-radius:${radius.full}px;
          border:1px solid ${colors.gray.borderMd};
          background:${colors.background.surface};
          font-size:${typography.scale.sm}px; font-weight:${typography.weight.semibold};
          cursor:pointer; color:${colors.gray[700]};
          transition:all ${transitions.fast}; white-space:nowrap;
          font-family:${typography.fontFamily};
        }
        .cp-prof:hover { border-color:${colors.red.borderHover}; color:${colors.red.DEFAULT}; }
        .cp-prof.sel {
          background:${colors.red.gradient}; color:#fff;
          border-color:transparent; box-shadow:${shadows.redSm};
        }
        .cp-svc {
          width:100%; display:flex; align-items:center; justify-content:space-between;
          padding:11px 14px; border-radius:${radius.sm}px;
          border:1px solid ${colors.gray.borderMd};
          background:${colors.background.surface};
        ...
        .cp-save {
          flex:1; padding:12px; border:none; border-radius:${radius.sm}px;
          font-size:${typography.scale.base}px; font-weight:${typography.weight.bold};
          cursor:pointer; transition:all ${transitions.base};
          font-family:${typography.fontFamily}; letter-spacing:.04em;
          background:${colors.red.gradient}; color:#fff;
          box-shadow:${shadows.redMd};
        }
        ...
        .cp-discard {
          padding:12px 16px; border:1px solid ${colors.gray.borderMd};
          border-radius:${radius.sm}px; background:${colors.background.surface};
          font-size:${typography.scale.sm}px; font-weight:${typography.weight.bold};
          cursor:pointer; color:${colors.gray[700]};
          font-family:${typography.fontFamily}; letter-spacing:.04em;
          transition:all ${transitions.fast}; white-space:nowrap;
        }
        .cp-discard:hover { border-color:${colors.gray.borderMd}; background:${colors.gray.hover}; }
        .cp-lbl {
          display:block; font-size:${typography.scale.xs}px;
          font-weight:${typography.weight.bold}; color:${colors.gray.dimText};
      ...
      {/* Painel lateral — renderizado via portal, z-index acima de tudo */}
      <div style={{
        position:'fixed', top:0, right:0, bottom:0,
        width:360, maxWidth:'100vw',
        background:glass.surface.modal.background,
        ...
        <div style={{ padding:'16px 20px 0', flexShrink:0 }}>
          ...
          {/* Pill cliente */}
          <div style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', borderRadius:radius.md, border:`1px dashed ${colors.gray.borderMd}`, background:colors.gray.hover, cursor:'pointer', marginBottom:14, transition:transitions.fast }}>
            <div style={{ width:34, height:34, borderRadius:radius.full, background:colors.background.surface, border:`1px solid ${colors.gray.border}`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
              <User size={15} color={colors.gray.dimText} />
            </div>
            <span style={{ fontSize:typography.scale.base, color:colors.gray.dimText, flex:1, lineHeight:1.3 }}>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'16px 20px' }}>
          ...
          {error && (
            <div style={{ marginBottom:12, padding:'9px 12px', borderRadius:radius.sm, background:'rgba(220,38,38,0.06)', border:`1px solid ${colors.red.border}`, color:colors.red.dark, fontSize:typography.scale.sm }}>
              {error}
            </div>
          )}
          {success && (
            <div style={{ marginBottom:12, padding:'9px 12px', borderRadius:radius.sm, background:'rgba(22,163,74,0.06)', border:'1px solid rgba(22,163,74,0.2)', color:'#15803d', fontSize:typography.scale.sm, display:'flex', alignItems:'center', gap:6 }}>
              <Check size={13} /> Agendamento confirmado!
            </div>
          )}
              ...
              {professionals.length > 1 && (
                <div style={{ marginBottom:14 }}>
                  ...
                  <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                    {professionals.map(p => (
                      <button key={p.id} className={`cp-prof${selectedProf===p.id?' sel':''}`} onClick={() => setSelectedProf(p.id)}>
                        {p.name}
                      </button>
                    ))}
                  </div>
        ...
        <div style={{ padding:'12px 20px 20px', borderTop:`1px solid ${colors.gray.border}`, flexShrink:0, background:colors.background.surface, backdropFilter:glass.blur.sm }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:12 }}>
            <div>
              ...
              <div style={{ fontSize:22, fontWeight:typography.weight.bold, color:colors.gray[900], fontVariantNumeric:'tabular-nums' }}>
