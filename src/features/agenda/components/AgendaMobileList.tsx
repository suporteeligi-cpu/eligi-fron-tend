...
export default function AgendaMobileList({ professionals, bookings }: Props) {
  ...
  // Agrupa os slots: slots ocupados aparecem como 1 item (só no startMin do booking)
  // Slots livres em sequência são exibidos individualmente
  const slotItems = useMemo(() => {
    return slots.reduce<{ time: string; booking?: AgendaBooking; free: boolean }[]>((acc, time) => {
      ...
      // Se está no meio de um booking, suprime o slot
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: colors.background.page, fontFamily: '-apple-system, "SF Pro Display", system-ui, sans-serif' }}>
      <style>{`
        ...
        .slot-free { display:flex; align-items:center; gap:12px; padding:10px 14px; border-radius:12px; background:rgba(255,255,255,0.5); border:1px dashed ${colors.gray.borderMd}; cursor:pointer; position:relative; overflow:hidden; transition:${transitions.base}; }
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 16px 10px' }}>
        ...
        <button className="new-btn" onClick={() => openCreate('09:00', currentProf ?? professionals[0]?.id ?? '')}>+ Novo</button>
      </div>

      {/* Lista de slots */}
      <div style={{ padding: '4px 16px 32px', display: 'flex', flexDirection: 'column', gap: 5 }}>
        {slotItems.map((item) => {
          if (item.booking) {
            ...
            return (
              <div key={`booked-${item.time}`} className="slot-booked" onClick={() => openCreate(item.booking!.start, item.booking!.professionalId)}>
          ...
          return (
            <div key={`free-${item.time}`} className="slot-free" onClick={() => openCreate(item.time, currentProf ?? professionals[0]?.id ?? '')}>
              <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 3, background: colors.gray.border }} />
              <span style={{ fontSize: 11, fontWeight: 500, color: colors.gray.dimTextLight, minWidth: 42, marginLeft: 8, fontVariantNumeric: 'tabular-nums' }}>{item.time}</span>
              <div style={{ width: 28, height: 28, borderRadius: '50%', background: colors.gray.hover, color: colors.gray.dimText, fontSize: 18, fontWeight: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>+</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 12, fontWeight: 400, color: colors.gray['500'] }}>Horário livre · {item.time}</div>
              </div>
            </div>
          )
        })}
