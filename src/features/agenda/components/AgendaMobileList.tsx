...
function MobileBookingCard({ booking, height }: { booking: AgendaBooking; height: number }) {
  ...
  return (
    <div style={{ width:'100%', height:'100%', borderRadius:8, background:gradient, padding:isTiny?'0 7px 0 9px':'5px 7px 5px 9px', display:'flex', flexDirection:'column', justifyContent:isTiny?'center':'flex-start', gap:2, overflow:'hidden', boxSizing:'border-box', boxShadow:`0 3px 10px ${glow}`, border:'1px solid rgba(255,255,255,0.15)', position:'relative', userSelect:'none', transition:'transform 0.12s ease, box-shadow 0.12s ease' }}>
      ...
      {isTiny ? (
        <div style={{ display:'flex', alignItems:'center', gap:4, overflow:'hidden', lineHeight:1 }}>
          <span style={{ fontSize:10, fontWeight:700, opacity:0.9, fontVariantNumeric:'tabular-nums', whiteSpace:'nowrap', flexShrink:0, color:'#fff' }}>{booking.start}–{booking.end}</span>
          <span style={{ fontSize:10, fontWeight:600, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', color:'#fff' }}>
            {booking.clientName}{booking.serviceName ? <span style={{ fontWeight:400, opacity:0.85 }}> · {booking.serviceName}</span> : null}
          </span>
        </div>
      ) : (
        <>
          <div style={{ color:'#fff', fontWeight:700, fontSize:12, lineHeight:1.2, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{booking.clientName}</div>
          {showService && <div style={{ color:'rgba(255,255,255,0.88)', fontWeight:500, fontSize:10, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{booking.serviceName}</div>}
          {showTime && (
...
export default function AgendaMobileList({ professionals, bookings, blocks, onDeleteBlock, onUpdateBlock }: Props) {
  ...
  useEffect(() => {
    if (currentY > 0 && vScrollRef.current) vScrollRef.current.scrollTop = Math.max(0, currentY - 80)
  }, [currentY])
      ...
      <div ref={vScrollRef} className="m-vscroll" style={{ flex:1, overflowY:'auto', overflowX:'auto', WebkitOverflowScrolling:'touch', position:'relative' }}>
        <div style={{ display:'flex', minHeight:TOTAL_H+COL_HEADER, width:`${TIME_COL_W + professionals.length * (COL_W + 1)}px` }}>
          ...
          {professionals.map((p, colIdx) => {
            ...
            return (
              <div key={p.id} ref={el => { colRefs.current[colIdx] = el }} style={{ width:COL_W, flexShrink:0, position:'relative', borderRight:`1px solid ${colors.gray.border}` }} onTouchMove={e => onColTouchMove(e, p.id, colIdx)} onTouchEnd={onColTouchEnd}>
                ...
                <div style={{ position:'relative', height:TOTAL_H }}>
                  ...
                  {/* Bloqueios — toque abre BlockEditModal */}
                  {profBlocks.map(bl => {
                  ...
                  {profBookings.map(b => {
                    ...
                    return (
                      <div key={b.id} style={{ position:'absolute', top: isThisDrag?drag!.ghostTop:top, left:cardLeft, width:cardWidth, height, zIndex:isThisDrag?40:8, opacity:isSaving?0.5:1, transition:isThisDrag?'none':'top 0.12s ease, opacity 0.2s', touchAction:'none', filter:isThisDrag?'drop-shadow(0 6px 18px rgba(0,0,0,0.3))':'none', transform:isThisDrag?'scale(1.02)':'scale(1)' }}
