...
export default function AgendaToolbar({ onBlockClick }: Props) {
  ...
  return (
    <div style={{
      ...
      <style>{`
        .toolbar-day-btn { flex:1 0 auto; min-width:46px; display:flex; flex-direction:column; align-items:center; padding:10px 4px; border-radius:14px; border:1px solid ${colors.gray.border}; cursor:pointer; background:rgba(255,255,255,0.7); backdrop-filter:blur(8px); transition:${transitions.spring}; }
        ...
        .toolbar-nav-btn { width:30px; height:30px; border-radius:50%; border:1px solid ${colors.gray.borderMd}; background:rgba(255,255,255,0.6); cursor:pointer; font-size:16px; display:flex; align-items:center; justify-content:center; color:${colors.gray['700']}; transition:${transitions.fast}; }
        ...
        .today-btn { padding:5px 13px; border-radius:20px; font-size:12px; font-weight:600; cursor:pointer; transition:${transitions.spring}; border:1px solid ${colors.red.border}; background:${colors.red.subtle}; color:${colors.red.DEFAULT}; }
        ...
        .today-btn.current { background:rgba(0,0,0,0.04); border-color:${colors.gray.borderMd}; color:${colors.gray['500']}; cursor:default; pointer-events:none; }
        .block-btn { display:flex; align-items:center; gap:5px; padding:5px 12px; border-radius:20px; font-size:12px; font-weight:600; cursor:pointer; transition:${transitions.spring}; border:1px solid rgba(71,85,105,0.2); background:rgba(71,85,105,0.06); color:#475569; }
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'20px 20px 16px' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          ...
          {/* Botão Hoje */}
          <button className={`today-btn${isToday?' current':''}`} onClick={() => setSelectedDate(today.toDate())}>
            Hoje
          </button>
