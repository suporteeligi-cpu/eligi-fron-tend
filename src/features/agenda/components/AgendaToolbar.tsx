...
// Toolbar com seletor de dias, calendário e ações (Novo / Bloquear / Hoje).
...
function CalendarPicker({ date, onSelect, onClose, isMobile }: {
  ...
  // SSR safety — portal só no cliente
  ...
  const content = (
    <>
      <div onClick={onClose} style={{
        ...
        background:'rgba(0,0,0,0.22)', backdropFilter:'blur(6px)', zIndex:9998,
      }} />
      ...
      <div style={isMobile ? {
        ...
        animation: `calUp 0.28s ${EASE.sheet}`,
        ...
        zIndex:9999, fontFamily: typography.fontFamily,
        animation: `calIn 0.22s ${EASE.spring}`,
        overflow:'hidden',
      }}>
        <style>{`
          ...
          .cal-day{display:flex; align-items:center; justify-content:center; width:36px; height:36px; border-radius:50%; cursor:pointer; font-size:13px; font-weight:500; transition:all 0.12s ${EASE.smooth}; border:none; background:transparent; -webkit-tap-highlight-color:transparent}
          .cal-day:active{transform:scale(0.88)}
          .cal-jump-btn{flex:1; padding:8px 0; border:1px solid ${colors.gray.borderMd}; border-radius:8px; background:${colors.background.surface}; font-size:12px; font-weight:600; cursor:pointer; color:${colors.gray[700]}; transition:all 0.12s ${EASE.smooth}; -webkit-tap-highlight-color:transparent}
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 20px 10px' }}>
          <button
            ...
            style={{
              ...
              border:`1px solid ${colors.gray.borderMd}`, background:'transparent',
          ...
          <button
            ...
            style={{
              ...
              letterSpacing:'-0.3px', padding:'4px 8px', borderRadius:8,
              transition: transitions.fast,
            }}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', padding:'0 16px', marginBottom:4 }}>
          ...
          {DAYS_HEADER.map(d => (
            <div key={d} style={{
              ...
              color: colors.gray.dimText, letterSpacing:'.06em', padding:'4px 0',
            }}>{d}</div>
          ))}
        </div>
            ...
            return (
              <div key={row} style={{ display:'flex', alignItems:'center', gap:0, marginBottom:2 }}>
                <div style={{
                  ...
                  color: hasSelDay ? colors.red.DEFAULT : 'rgba(0,0,0,0.22)',
                  letterSpacing:'.02em',
                }}>{weekNum}</div>
        ...
        <div style={{ padding:'0 16px 4px' }}>
          ...
          }}>Pular por semana</div>
          <div style={{ display:'flex', gap:4, marginBottom:4 }}>
            {[1,2,3,4,5,6].map(n => (
              <button key={n} className="cal-jump-btn" onClick={() => { onSelect(date.add(n,'week')); onClose() }}>
                +{n}
              </button>
            ))}
          </div>
          <div style={{ display:'flex', gap:4 }}>
            {[1,2,3,4,5,6].map(n => (
              <button key={-n} className="cal-jump-btn" onClick={() => { onSelect(date.subtract(n,'week')); onClose() }}
                style={{ color: colors.gray.dimText }}
              >
                -{n}
              </button>
            ))}
        ...
        <div style={{ padding:'10px 16px 0', display:'flex', justifyContent:'flex-end' }}>
          <button onClick={onClose} style={{
            ...
            border:`1px solid ${colors.gray.borderMd}`, background:'transparent',
            fontSize:13, fontWeight:600, cursor:'pointer', color: colors.gray[700],
          }}>
            Fechar
          </button>
        </div>
...
export default function AgendaToolbar({ onBlockClick, professionals }: Props) {
  ...
  // FIX #10: scrollIntoView só após interação do usuário (não no mount)
  ...
  // FIX #11: "Novo" abre com o primeiro profissional (não vazio)
                ...
                return (
                  ...
                  >
                    <span style={{
                      ...
                      color: isSel ? 'rgba(255,255,255,0.7)' : colors.gray.dimText, marginBottom:2,
                    }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8, padding:'0 20px 10px' }}>
            ...
            <div style={{ flex:1, display:'flex', alignItems:'center', gap:4 }}>
              ...
              <div ref={stripRef} className="strip-scroll" style={{
                flex:1, display:'flex', gap:4, overflowX:'auto', scrollbarWidth:'none',
                  ...
                  return (
                    ...
                    >
                      <span style={{
                        ...
                        color: isSel ? 'rgba(255,255,255,0.75)' : colors.gray.dimText, marginBottom:3,
                      }}>
                        {DAYS_PT[day.day()].toUpperCase()}
                      </span>
                      <span style={{
