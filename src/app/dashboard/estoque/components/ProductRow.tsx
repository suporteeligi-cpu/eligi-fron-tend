...
// - Se trackStock=true: badge de status + saldo + botão de atalho rápido de movimentação

import { Package, ChevronRight, ArrowDownUp } from 'lucide-react'
...
interface Props {
  ...
  isLast?:    boolean
  onClick:    () => void
  onQuickMove?: (p: Product) => void   // atalho rápido de movimentação
}

export default function ProductRow({ product, isMobile, isLast, onClick, onQuickMove }: Props) {
  ...
  return (
    ...
    >
      {/* Área clicável principal (abre edição) */}
      <button
        onClick={onClick}
        style={{
          flex: 1,
      ...
      {/* Atalho rápido de movimentação (só pra produtos com tracking) */}
      {tracking && onQuickMove ? (
        <button
          onClick={() => onQuickMove(product)}
        ...
        isMobile && <ChevronRight size={15} color={colors.gray.dimText} strokeWidth={2} style={{ flexShrink: 0 }} />
      )}
    </div>
  )
}
