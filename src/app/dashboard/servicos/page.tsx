    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ flex:1, overflowY:'auto', padding:20 }}>
          ...
          <div style={{ marginBottom:16 }}>
            ...
            <div style={{ display:'flex', gap:8 }}>
              {[{ val: durationH, opts: HOUR_OPTS, label: 'h', set: (v: number) => setDuration(v * 60 + durationM) },
                { val: durationM, opts: MIN_OPTS,  label: 'min', set: (v: number) => setDuration(durationH * 60 + v) }
              ].map(({ val, opts, label, set }) => (
        ...
        <div style={{ padding:'14px 20px 20px', borderTop:'1px solid rgba(0,0,0,0.07)', display:'flex', gap:8, flexShrink:0 }}>
          <button onClick={handleSave} disabled={saving} style={{ flex:1, padding:13, background:saving?'rgba(220,38,38,0.25)':'linear-gradient(135deg,#dc2626,#b91c1c)', color:'#fff', border:'none', borderRadius:12, fontWeight:600, fontSize:15, cursor:saving?'not-allowed':'pointer', boxShadow:saving?'none':'0 4px 16px rgba(220,38,38,0.28)', transition:'all .2s' }}>
